    ...
    for l in lines:
        if TEXTBOUND_RE.search(l):
            ...
            if mult_sent:
                ...
                for end in ends[:-1]:
                    n = end - start
                    #assert text[n].isspace(),[l,text[n],start_ends ]#velvin
                    text[n] = '\n'
                text = ''.join(text)
        ...
        if ATTRIBUTE_RE.search(l):
            ...
            if tb_id in attributes:
                ...
                # raise error
                    ...
                    logging.warn("Existing attribute: {}".format(attributes[tb_id]))
                    logging.warn("New attribute:      {}".format(attr_ob))
...
def get_filename(path):
    root, ext = os.path.splitext(path) #filename ann/txt
    return root
