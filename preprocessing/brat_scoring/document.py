...
from brat_scoring.labels import tb2entities, tb2relations, brat2events,Relation
...
class Document:
    ...
    def event_relations(self, by_sent=False, entity_types=None):
        '''
        get list of event_relations for document
        '''
        relations=[]
        events=self.events(as_dict = True)
        #add more relations from events #velvin
        for id, relation_brat in self.relation_dict.items():
            tb_1 = relation_brat.arg1
            tb_2 = relation_brat.arg2
            role = relation_brat.role
            
            if (tb_1[0] == 'E') or (tb_2[0] == 'E'):
                assert tb_1 in events, f'doc: {self.id}, reltation tb {tb_1} not in entities {events.keys()}'
                assert tb_2 in events, f'doc: {self.id}, reltation tb {tb_2} not in entities {events.keys()}'

                relation = Relation( \
                            entity_a = copy.deepcopy(events[tb_1]),
                            entity_b = copy.deepcopy(events[tb_2]),
                            role = role)
    ...
    def events(self, by_sent=False, event_types=None, entity_types=None,as_dict = False):
        '''
        get list of entities for document
        '''
