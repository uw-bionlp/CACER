...
def divide(l1,l2):
    ...
    return [a/b if b!=0.0 else float("inf") for a,b in zip(l1,l2) ]
def PRF(df):
    print(len(df[C.NP].astype(float)))
    df["P"] = divide(df[C.TP],(df[C.NP].astype(float)))# if df[C.NP].astype(float)>0 else 
    df["R"] = divide(df[C.TP],(df[C.NT].astype(float)))# if df[C.NT].astype(float)>0 else float("inf")
    df["F1"] = divide(2*df["P"]*df["R"],(df["P"] + df["R"]))
...
def get_path(path, description=None, ext='.csv', name='scores'):
    ...
    if ext in path:
        ...
        f =path+ f'{name}{ext}'# os.path.join(path, f'{name}{ext}')
    else:
        f = path, f"{name}_{description}{ext}" #os.path.join()
